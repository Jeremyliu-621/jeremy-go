...
export function CatchMeter({ phase, accuracy, catchRate }: Props) {
  const showMeter = accuracy !== null && (
    phase === 'absorbing' || phase === 'wobbling' || phase === 'success' || phase === 'escaped'
  )
  ...
  const barColor =
    clampedRate >= 0.7 ? '#78C850'
    : clampedRate >= 0.4 ? '#F8D030'
    : '#F08030'

  const label =
    clampedRate >= 0.7 ? 'Excellent!'
    : clampedRate >= 0.5 ? 'Great'
    : clampedRate >= 0.3 ? 'Nice'
    : 'Tricky...'
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <MiniPokeball success={phase === 'success'} wobbling={phase === 'wobbling'} />
        </div>
      </motion.div>
    </AnimatePresence>
  )
...
function MiniPokeball({ success, wobbling }: { success: boolean; wobbling: boolean }) {
  return (
    <motion.div
      animate={
        success
    ...
    >
      <svg width="24" height="24" viewBox="0 0 100 100">
        ...
        <g clipPath="url(#meter-ball-clip)">
          <rect x="0" y="0" width="100" height="50" fill={success ? '#FFCB05' : '#FF4444'} />
